...
def _reverse_geocode(lat, lng):
  ...
  try:
    ...
    req = urllib.request.Request(
      url,
      headers={
        # Nominatim yêu cầu User-Agent rõ ràng
...
def lambda_handler(event, context):
  ...
  try:
    ...
    # Location name: ưu tiên FE gửi, nếu không có thì BE tự reverse geocode
    ...
    # Nếu không có location_name từ FE → gọi Nominatim ở backend
    if not location_name:
      auto_loc = _reverse_geocode(lat_f, lng_f)
      if auto_loc:
        location_name = auto_loc
    ...
    if valid_image_keys:
      item["imageKeys"] = valid_image_keys 
      item["imageKey"] = cover_image_key
