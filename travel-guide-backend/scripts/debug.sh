...
echo "6. Checking Lambda function files..."
FUNCTION_DIRS=(
...
echo "7. Checking existing CloudFormation stacks..."
STACKS=(
    ...
    "travel-guide-ai-service-staging"
)
for stack in "${STACKS[@]}"; do
    echo "   - Checking $stack..."
    if aws cloudformation describe-stacks --stack-name "$stack" --query 'Stacks[0].StackStatus' --output text &> /dev/null; then
...
echo "8. Checking S3 deployment bucket..."
...
if aws s3 ls "s3://$DEPLOY_BUCKET" &> /dev/null; then
    ...
    echo "   Contents:"
    aws s3 ls "s3://$DEPLOY_BUCKET" --recursive | head -10
else
