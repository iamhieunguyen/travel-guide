...
###################################
# Travel Guide - Deploy Notification #
###################################
...
cleanup() {
  ...
  if [[ $exit_code -ne 0 ]]; then
    echo ""
    echo "========================================"
    echo "🚨 DEPLOY NOTIFICATION SERVICE FAILED"
    echo "========================================"
...
log "🚢 Deploy NOTIFICATION SERVICE"
log "  ENV          : $ENV"
log "  REGION       : $REGION"
log "  PROFILE      : $PROFILE"
...
log "🔧 sam build (notification-service)..."
...
echo "    - SenderEmail=noreply@travelguide.com"
echo ""
...
log "🚢 sam deploy (notification-service)..."
sam deploy \
  --stack-name "$STACK_NAME" \
  ...
  --parameter-overrides \
    ...
    "Environment=$ENV" \
    "SenderEmail=noreply@travelguide.com"

log "✅ NOTIFICATION SERVICE deployed successfully"
