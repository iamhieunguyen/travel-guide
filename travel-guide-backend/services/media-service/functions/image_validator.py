...
import io
from utils.cors import error  # Cập nhật đường dẫn import

# Cập nhật đường dẫn để import các module shared
sys.path.insert(0, '/opt/python/utils')  # Lambda layer path
sys.path.insert(0, '/var/task/functions')  # Current function path

s3_client = boto3.client('s3')
sqs_client = boto3.client('sqs')
dynamodb = boto3.resource('dynamodb')

# Sử dụng environment variables từ template
TABLE_NAME = os.environ.get('TABLE_NAME', '')
BUCKET_NAME = os.environ.get('BUCKET_NAME', '')
IMAGE_ANALYZER_QUEUE_URL = os.environ.get('IMAGE_ANALYZER_QUEUE_URL', '')

# Đọc các cấu hình từ environment variables
