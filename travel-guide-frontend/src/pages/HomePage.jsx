...
export default function HomePage() {
  ...
  const loadPosts = useCallback(
    async (token = null, query = '') => {
      try {
        ...
        if (isSearchMode && api.searchArticles) {
          // Đang search → dùng API /search nếu có
          ...
          // Không search (hoặc chưa có searchArticles) → list bình thường
        ...
        // Filter thêm ở FE để search theo location/title/content
        let filteredItems = items;
        if (isSearchMode) {
          const searchTerm = trimmed.toLowerCase();
          filteredItems = items.filter((post) => {
            const locationText = (
              post.locationName ||
              post.location?.name ||
              post.location ||
  ...
  const handleSearch = (e) => {
    if (e.key === 'Enter' || e.type === 'click') {
      // reset về trang đầu, dùng query hiện tại
                ...
                <div className="space-y-8">
                  {loading && posts.length === 0 ? (
                    [...Array(3)].map((_, i) => (
                      <div
                        key={i}
                        className="bg-white rounded-3xl shadow-sm p-5 animate-pulse"
                      >
                    ...
                    <>
                      {posts.map((post) => {
                        ...
                        return (
                          <div
                            key={post.articleId}
                            className="max-w-2xl mx-auto bg-white rounded-[32px] shadow-lg overflow-hidden p-6"
                          >
                            {/* User Info */}
                              ...
                              <div className="flex items-center space-x-3">
                                ...
                                <div>
                                  <p className="font-bold text-gray-800 text-base">
                                    {post.username ||
                                      `User_${post.ownerId?.substring(0, 6)}`}
                                  </p>
                                  {locationText && (
                                    <div
                                ...
                                <div className="relative">
                                  ...
                                  {openMenuId === post.articleId && (
                                    <>
                                      ...
                                      />
                                      <div
                                        className="absolute right-0 mt-2 w-48 bg-white rounded-2xl shadow-xl z-[9999] overflow-hidden"
                                        style={{
                                          border:
                                      ...
                                      >
                                        ...
                                        >
                                          ...
                                          >
                                            <path
                                              ...
                                              strokeWidth={2}
                                              d="M11 5H6a 2 2 0 00-2 2v11a 2 2 0 002 2h11a 2 2 0 002-2v-5m-1.414-9.414a 2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"
                                            />
                                          </svg>
                                          <span>Chỉnh sửa</span>
                                        </button>
                                        <button
                                          onClick={() =>
                                            handleDeletePost(post.articleId)
                                          }
                                        ...
                                        >
                                          ...
                                          >
                                            <path
                                              ...
                                              strokeLinejoin="round"
                                              strokeWidth={2}
                                              d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a 2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a 1 1 0 00-1-1h-4a 1 1 0 00-1 1v3M4 7h16"
                                            />
                                          </svg>
                                          <span>Xóa</span>
                                        </button>
