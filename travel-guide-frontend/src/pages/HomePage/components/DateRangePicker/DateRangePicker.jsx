...
export default function DateRangePicker({ selected, onSelect, className = '' }) {
  ...
  useEffect(() => {
    if (isOpen && triggerRef.current) {
      ...
      const popupHeight = 400; // Ước tính chiều cao của popup
      const popupWidth = 350; // Ước tính chiều rộng của popup
      ...
      if (top + popupHeight > viewportHeight - 20) {
        ...
        // Nếu vẫn không đủ chỗ, đặt ở giữa màn hình
        if (top < 20) {
      ...
      let left = rect.left;
      // Đảm bảo popup không bị tràn ra ngoài màn hình bên phải
      if (left + popupWidth > viewportWidth - 20) {
        left = viewportWidth - popupWidth - 20;
      }
      // Đảm bảo popup không bị tràn ra ngoài màn hình bên trái
      if (left < 20) {
        left = 20;
      }
